// scripts/sync-archive.js
// 🎯 V50 Logic for Archive (2020 -> 2010)
// იგივე ძლიერი ძებნა, ოღონდ ძველი წლებისთვის.
...
// 💡 ცვლილება 1: პროგრესის ფაილი სხვაა, რომ მთავარს არ შეუშალოს ხელი
const PROGRESS_FILE = path.join(process.cwd(), 'scripts', 'sync-archive-progress.json');

// 💡 ცვლილება 2: წლები 2020-დან 2010-მდე
const TARGET_YEARS = [2020, 2019, 2018, 2017, 2016, 2015, 2014, 2013, 2012, 2011, 2010];
...
async function saveMovie(client, kinobdItem, tmdbItem, fallbackTrailer, omdbData, kpIdExternal) {
    if (!tmdbItem || !tmdbItem.id) return;
...
async function main() {
  ...
  if (args.includes('--reset')) {
      try { if (fs.existsSync(PROGRESS_FILE)) fs.unlinkSync(PROGRESS_FILE); console.log("🔄 Archive Reset!"); } catch(e) {}
  }

  let savedState = loadProgress();
  console.log(`🚀 იწყება არქივის სინქრონიზაცია (2020-2010)...`);
