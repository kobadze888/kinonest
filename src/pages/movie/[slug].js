...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    if (movieRes.rows.length > 0) {
      ...
      // 3. რეკომენდაციები (მკაცრი ფილტრით: წელი, ენა, პოსტერი)
      if (movie.genres_names && movie.genres_names.length > 0) {
        try {
            ...
            const recRes = await query(`
                ...
                WHERE type = 'movie'
                  AND tmdb_id != $1
                  AND title_ru ~ '[а-яА-ЯёЁ]'      -- მხოლოდ რუსული სათაური
                  AND poster_path IS NOT NULL      -- პოსტერი აუცილებელია
                  AND kinopoisk_id IS NOT NULL     -- პლეერი სასურველია
                  AND release_year = $3            -- 💡 იგივე წელი
                  ${genreFilter}
      ...
      <section className="relative h-[60vh] md:h-[70vh] min-h-[400px] w-full">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-end pb-12">
          <div className="w-full md:w-2/3 lg:w-1/2">
            ...
            <p className="max-w-xl text-sm md:text-base text-gray-200 mt-3 line-clamp-3">
              {movie.overview}
            </p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-8 flex flex-col h-full">
            {/* 💡 ცვლილება აქ: mb-6 შეიცვალა mb-2-ით 
                ეს შეამცირებს დაშორებას მსახიობებსა და დეტალებს შორის.
            */}
