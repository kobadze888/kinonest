...
export async function getServerSideProps(context) {
  ...
  let movie = null, kinopoisk_id = null, actors = [], recommendations = [];

  try {
    const movieRes = await query(`SELECT * FROM media WHERE tmdb_id = $1`, [tmdbId]);
    if (movieRes.rows.length > 0) {
      movie = movieRes.rows[0];
      kinopoisk_id = movie.kinopoisk_id;
      movie.formattedBudget = movie.budget > 0 ? `$${Number(movie.budget).toLocaleString('en-US')}` : '-';
      movie.formattedPremiere = movie.premiere_world ? new Date(movie.premiere_world).toLocaleDateString('ru-RU') : '-';
      ...
      if (movie.genres_names && movie.genres_names.length > 0) {
          ...
          const recRes = await query(`SELECT tmdb_id, title_ru, poster_path, rating_tmdb, release_year, type FROM media WHERE type = 'movie' AND tmdb_id != $1 AND title_ru ~ '[а-яА-ЯёЁ]' AND poster_path IS NOT NULL AND kinopoisk_id IS NOT NULL AND release_year = $3 ${genreFilter} AND genres_names && $2::text[] ORDER BY rating_tmdb DESC, popularity DESC LIMIT 15`, [tmdbId, movie.genres_names, movie.release_year]);
  ...
  return { props: { movie: JSON.parse(JSON.stringify(movie)), kinopoisk_id, actors, recommendations, isAdmin } };
}
...
export default function MoviePage({ movie, kinopoisk_id, actors, recommendations, isAdmin }) {
  ...
  const schemaData = {
    "@context": "https://schema.org", "@type": "Movie", "name": title, "alternativeHeadline": movie.title_en, "image": posterPath, "description": movie.overview,
    "datePublished": movie.premiere_world || `${releaseYear}-01-01`, "aggregateRating": { "@type": "AggregateRating", "ratingValue": movie.rating_tmdb || movie.rating_kp || 0, "bestRating": "10", "ratingCount": movie.rating_imdb_count > 0 ? movie.rating_imdb_count : 50 },
    "actor": actors.slice(0, 5).map(actor => ({ "@type": "Person", "name": actor.name })), "genre": movie.genres_names, "offers": { "@type": "Offer", "availability": "https://schema.org/InStock", "price": "0", "priceCurrency": "RUB" }
  };
  ...
  return (
    <div className="bg-[#10141A] text-white font-sans min-h-screen flex flex-col">
      ...
      {kinopoisk_id && <section className="bg-[#10141A] pt-24 md:pt-32 pb-0 relative z-20"><PlayerContainer kinopoisk_id={kinopoisk_id} imdb_id={movie.imdb_id} tmdb_id={movie.tmdb_id} title={title} trailer_url={movie.trailer_url} type="movie" /></section>}
      ...
      <section className="relative h-[45vh] w-full lg:hidden -mt-2 z-10">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 z-10 bg-gradient-to-t from-[#10141A] to-transparent pt-12">
          ...
          <div className="flex items-center gap-3 mt-2 text-sm text-gray-300">
            ...
            <span>{releaseYear}</span><span>{movie.runtime ? `${movie.runtime} мин` : ''}</span>
          </div>
        </div>
      </section>
      ...
      <div className="lg:hidden px-4 pb-10 space-y-6 -mt-2 relative z-20">
        <div className="flex gap-4">
          <div className="w-28 flex-shrink-0 rounded-lg overflow-hidden shadow-lg border border-gray-800 relative aspect-[2/3]">
            ...
            {isAdmin && <Link href={`/admin/edit/${movie.tmdb_id}`} target="_blank" className="absolute top-2 right-2 z-20 flex items-center justify-center p-1.5 rounded-full bg-red-800/80 text-white hover:bg-brand-red transition-colors shadow-md border border-white/10"><EditIcon /></Link>}
          </div>
          <div className="flex-grow flex flex-col justify-center gap-3">
            ...
            <div className="flex flex-wrap gap-1.5">{(movie.genres_names || []).slice(0, 2).map((g, i) => (<Link key={i} href={`/discover?genre=${g.toLowerCase()}`} className="text-xs bg-gray-800 px-2 py-1 rounded text-gray-300 border border-gray-700 hover:bg-brand-red hover:text-white hover:border-brand-red transition-colors">{g}</Link>))}</div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button onClick={handleShowTrailerModal} className="col-span-2 bg-brand-red text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-transform"><PlayIcon /> Смотреть трейлер</button>
          <button onClick={() => toggleItem(movie.tmdb_id)} className={`py-3 rounded-xl font-bold border flex items-center justify-center gap-2 active:scale-95 transition-transform ${isFavorite ? 'bg-white/10 border-brand-red text-brand-red' : 'border-gray-600 text-gray-300'}`}><HeartIcon isFilled={isFavorite} /> {isFavorite ? 'В избранном' : 'В избранное'}</button>
          <Link href="/movies" className="py-3 rounded-xl font-bold border border-gray-600 text-gray-300 flex items-center justify-center gap-2 active:scale-95 transition-transform"><FilmIcon /> ФИЛЬМ</Link>
        </div>
        ...
        <div className="bg-[#151a21] rounded-xl p-4 border border-gray-800"><div className="grid grid-cols-2 gap-y-4 text-xs"><div><span className="block text-gray-500 mb-1">Бюджет</span><span className="text-white">{movie.formattedBudget}</span></div><div><span className="block text-gray-500 mb-1">Премьера</span><span className="text-white">{movie.formattedPremiere}</span></div></div></div>
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-8 h-full flex items-end pb-12">
            <div className="w-1/2">
              ...
              <div className="flex items-center space-x-3 mt-5">
                ...
                <Link href="/movies" className="flex items-center gap-2 px-6 py-2.5 rounded-lg font-bold transition border-2 border-gray-500 text-gray-300 hover:text-white hover:border-white hover:bg-white/5 cursor-pointer"><FilmIcon /> ФИЛЬМ</Link>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-12 gap-8 items-stretch">
              ...
              <div className="bg-[#151a21] border border-gray-800 rounded-xl p-6 shadow-lg flex-grow flex flex-col justify-center">
                ...
                <div className="grid grid-cols-3 gap-y-6 gap-x-4 text-sm">
                  ...
                  <div className="col-span-3 pt-2">
                    ...
                    <div className="flex flex-wrap gap-2">{(movie.genres_names || []).map((g, i) => (<Link key={i} href={`/discover?genre=${g.toLowerCase()}`} className="px-3 py-1 bg-gray-800 text-gray-300 rounded-md border border-gray-700 hover:bg-brand-red hover:text-white hover:border-brand-red transition-colors cursor-pointer">{g}</Link>))}</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-span-4 h-full">
              <div className="relative rounded-xl overflow-hidden shadow-2xl border-4 border-gray-800/50 w-full h-full min-h-[500px]">
                ...
                {isAdmin && <Link href={`/admin/edit/${movie.tmdb_id}`} target="_blank" className="absolute top-4 right-4 z-20 flex items-center justify-center p-2.5 rounded-full bg-red-800/80 text-white hover:bg-brand-red transition-colors shadow-xl border border-white/20"><EditIcon /></Link>}
              </div>
            </div>
          </div>
