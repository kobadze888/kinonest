...
export async function getServerSideProps(context) {
  ...
  return {
    props: { 
      tvShow: JSON.parse(JSON.stringify(tvShow)), 
      kinopoisk_id, 
      actors, 
      recommendations 
    },
      ...
      <section className="relative h-[45vh] w-full lg:hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 z-10 bg-gradient-to-t from-[#10141A] to-transparent pt-12">
           ...
           <div className="flex items-center gap-3 mt-2 text-sm text-gray-300">
              ...
              <span>{tvShow.runtime ? `~ ${tvShow.runtime} мин` : ''}</span>
           </div>
        </div>
      </section>
            ...
            <div className="flex-grow flex flex-col justify-center gap-3">
               <div className="flex items-center gap-2">
                  {tvShow.rating_imdb > 0 && (
                    <span className="bg-[#F5C518] text-black px-1.5 py-0.5 rounded font-bold text-xs">IMDb {tvShow.rating_imdb}</span>
                  )}
                  {tvShow.rating_kp > 0 && (
                    <span className="bg-[#f50] text-white px-1.5 py-0.5 rounded font-bold text-xs">KP {tvShow.rating_kp}</span>
                  )}
               </div>
               ...
               <div className="flex flex-wrap gap-1.5">
                  {(tvShow.genres_names || []).slice(0, 2).map((g, i) => (
                     <span key={i} className="text-xs bg-gray-800 px-2 py-1 rounded text-gray-300 border border-gray-700">{g}</span>
                  ))}
               </div>
            </div>
         </div>
         ...
         <div className="bg-[#151a21] rounded-xl p-4 border border-gray-800">
            <div className="grid grid-cols-2 gap-y-4 text-xs">
               <div>
                  ...
                  <span className="text-white">{tvShow.formattedPremiere}</span>
               </div>
            </div>
         </div>
